...
<body class="d-flex flex-column h-100">
   ...
   <div class="container-fluid">
    ...
    ?>
    <script src="/php-sql/assets/vendor/Chart.js/Chart.min.js"></script>
    <script>
        $(document).ready(function(){
            function getDuLieuBaoCaoTongSoMatHang(){
                $.ajax('/php-sql/backend/api/tongsomathang.php',{
                    ...
                    error: function(){
                        var htmlString ='<h1>Khong the xu ly</h1>';
                        $('#baocaoSanPham_SoLuong').html(htmlString);
                    }
                });
            }
            ...
            function getDuLieuBaoCaoTongSoKhachHang() {
                $.ajax('/php-sql/backend/api/tongsokhachhang.php', {
                success: function(data) {
                    var dataObj = JSON.parse(data);
                    var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      $('#refreshBaoCaoKhachHang').click(function(event) {
       ...
       function getDuLieuBaoCaoTongSoDonHang() {
        $.ajax('/php-sql/backend/api/tongsodonhang.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function getDuLieuBaoCaoTongSoGopY() {
        $.ajax('/php-sql/backend/api/tongsogopy.php', {
          success: function(data) {
            var dataObj = JSON.parse(data);
            var htmlString = `<h1>${dataObj.SoLuong}</h1>`;
      ...
      function renderChartThongKeLoaiSanPham() {
        $.ajax({
          url: '/php-sql/backend/api/thongkeloaisanpham.php',
          type: "GET",
